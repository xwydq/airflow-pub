...
# conn_hook = MySqlHook('hwybi')
# conn_hook.get_sqlalchemy_engine().table_names()
...
def get_csvfile_list():
    csvpath = dcmp_settings.CSV_DIR
    files = [f for f in glob.glob(csvpath + "**/*.csv", recursive=False)]
    files.extend([f for f in glob.glob(csvpath + "**/*.CSV", recursive=False)])
...
def command_render(task_type, command):
    attr_renderer = {
        ...
        'sql_operate': lambda x: render(x, lexers.SqlLexer),
...
class RequestArgsFilter(object):
    ...
    def refresh_filter_arg_dict(self):
        ...
        # print("filter_arg_dict", filter_arg_dict)
...
class DagCreationManager(BaseView):
    CONSTANT_KWS = {
        ...
        "TASK_TYPES_NAME": dcmp_settings.TASK_TYPES_NAME,
    ...
    def api(self, session=None):
        ...
        if api == "conn_list":
            ...
            # db_choices = [db.conn_id for db in dbs]
            return jsonify({"code": 0, "detail": {"conn_list": db_choices}, })
        elif api == "get_conntype":
            if not can_access_approver():
                return jsonify({"code": -100, "detail": "no permission", })
            ...
            if conn_id:
                ...
                if dbs:
                    ...
                    return jsonify({"code": 0, "detail": {"conn_type": db_type}, })
                else:
                    return jsonify({"code": -1, "detail": "conn_id invalid", })
        elif api == "get_tblnm_list":
            if not can_access_approver():
                return jsonify({"code": -100, "detail": "no permission", })
            conn_id = request.args.get("conn_id")
            return jsonify({"tblnm_list": get_tblnm_list(conn_id), })
...
# dag_creation_manager_view = DagCreationManager(category="指令集", name="DAG 管理")
...
# babel = Babel(dag_creation_manager_bp)
