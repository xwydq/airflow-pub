    ...
    var task_to_element_by_field = function(task_id, task, field_name, field_type, required, field_name_verbose, helper_text) {
        ...
        if(field_type == "queue_pool"){
            ...
            for(var i=0; i<task_types.length; i++){
                ...
                var task_type_name = task_types_name[i];
                ...
                field_html.push('<li role="presentation" field-value="' + task_type + '" class="' + (task_type == task[field_name]? 'active': '') + '"><a href="#command_' + task_id + '_' + task_type + '">' + task_type_name + '</a></li>');
            }
            field_html.push('</ul>');
            ...
            for(var i=0; i<task_types.length; i++){
                ...
                if(task_type == "time_sensor"){
                ...
                }else if(task_type == "partition_sensor"){
                    field_html.push('<textarea ' + (readonly? ' readonly="readonly" ': '') + ' id="ace_' + task_id + '_' + task_type + '" class="form-control" rows="1" name="command">' + (task_type == task[field_name]? task["command"]: '') + '</textarea>');
                    field_html.push('<p class="help-block">Each line is a partition name of the form schema.table/pk1=pv1/pk2=pv2. for example, default.users/ds=2016-01-01.</p>');
                    field_html.push(render_help);
                ...
                }else if(task_type == "sql_operate"){
                    field_html.push(render_help);
                    option_list = ' <div>\n' +
                        '          <label for="userId">数据连接：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') + 'id="' + 'conn_' + task_id + '_' + task_type + '" name="conn_id" class="form-control">';
                    for(var oi=0; oi<sql_conn_list.length; oi++){

                        if(sql_conn_list[oi] == task["conn_id"]){
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '" selected = "selected">' + sql_conn_list[oi]+ '</option>';
                        }else{
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '">' + sql_conn_list[oi]+ '</option>';
                        }

                    }
                    option_list=option_list + '</select>' + '</div>';
                    field_html.push(option_list);
                    field_html.push('<br>');

                    field_html.push('<textarea ' + (readonly? ' readonly="readonly" ': '') + ' id="ace_' + task_id + '_' + task_type + '" class="form-control" rows="1" name="command">' + (task_type == task[field_name]? task["command"]: '') + '</textarea>');
                    // field_html.push('<textarea ' + (readonly? ' readonly="readonly" ': '') + ' id="ace2_' + task_id + '_' + task_type + '" class="form-control" rows="1" name="command">' + (task_type == task[field_name]? task["command"]: '') + '</textarea>');

                    field_html.push(get_ace_script(task_type, "sql"));
                }else if(task_type == "data_transfer"){
                    // help info
                    field_html.push(render_help);

                    // source connection
                    option_list = ' <div>\n' +
                        '          <label for="userId">数据来源Conn：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') + 'id="' + 'src_conn_' + task_id + '_' + task_type + '" name="src_conn_id" class="form-control">';
                    for(var oi=0; oi<sql_conn_list.length; oi++){
                        if(sql_conn_list[oi] == task["src_conn_id"]){
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '" selected = "selected">' + sql_conn_list[oi]+ '</option>';
                        }else{
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '">' + sql_conn_list[oi]+ '</option>';
                        }
                    ...
                    option_list = ' <div>\n' +
                        '          <label for="userId">数据导入Conn：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') + 'id="' + 'dst_conn_' + task_id + '_' + task_type + '" name="dst_conn_id" class="form-control">';
                    for(var oi=0; oi<sql_conn_list.length; oi++){
                        if(sql_conn_list[oi] == task["dst_conn_id"]){
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '" selected = "selected">' + sql_conn_list[oi]+ '</option>';
                        }else{
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '">' + sql_conn_list[oi]+ '</option>';
                        }
                    ...
                    //destination table
                    dst_table_name = ' <div>\n' +
                        '          <label for="dstTabName">导入目标表名：</label>' +
                        '<input type="text" ' + (readonly? ' readonly="readonly" ': '') +
                        ' id="dst_tab_' + task_id + '_' + task_type +
                        '" class="form-control" rows="1" name="dst_tab_name"' +
                        'value=' + (task_type == task[field_name]? task["dst_tab_name"]: "") + '>' +
                        '</div>';
                    field_html.push(dst_table_name);
                    field_html.push('<br>');

                    // SQL
                    sql_oper = ' <div>\n' +
                        '          <label for="sqlOper">执行SQL：</label>' +
                        '<textarea ' + (readonly? ' readonly="readonly" ': '') +
                        ' id="ace_' + task_id + '_' + task_type +
                        '" class="form-control" rows="1" name="command">' +
                        (task_type == task[field_name]? task["command"]: '') +
                        '</textarea>' +
                        '</div>';
                    field_html.push(sql_oper);
                    field_html.push('<br>');
                    field_html.push(get_ace_script(task_type, "sql"));
                }else if(task_type == "data_upload"){
                    // field_html.push(render_help);
                    // csv_files
                    option_list = ' <div>\n' +
                        '          <label for="userId">CSV数据文件：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') +
                        'id="' + 'upload_csvfile_' + task_id + '_' + task_type +
                        '" name="upload_csvfile" class="form-control">';
                    for(var oi=0; oi<upload_csvfile_list.length; oi++){
                        if(upload_csvfile_list[oi] == task["upload_csvfile"]){
                            option_list=option_list + '<option value="' + upload_csvfile_list[oi]+ '" selected = "selected">' + upload_csvfile_list[oi]+ '</option>';
                        }else{
                            option_list=option_list + '<option value="' + upload_csvfile_list[oi]+ '">' + upload_csvfile_list[oi]+ '</option>';
                        }
                    }
                    option_list=option_list + '</select>' + '</div>';
                    field_html.push(option_list);
                    ...
                    // conn_id
                    // window.task_numi = task
                    // sql_conn_list
                    option_list = ' <div>\n' +
                        '          <label for="userId">导入数据源：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') +
                        // 'onchange="getval(this, taskd=task_numi);"' +
                        'id="' + 'upload_conn_' + task_id + '_' + task_type +
                        '" name="upload_conn_id" class="form-control">';
                    for(var oi=0; oi<sql_conn_list.length; oi++){
                        if(sql_conn_list[oi] == task["upload_conn_id"]){
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '" selected = "selected">' + sql_conn_list[oi]+ '</option>';
                        }else{
                            option_list=option_list + '<option value="' + sql_conn_list[oi]+ '">' + sql_conn_list[oi]+ '</option>';
                        }
                    ...
                    // tablenm_list
                    option_list = ' <div>\n' +
                        '          <label for="userId">导入数据表名：</label>' +
                        '<select ' + (readonly? ' disabled="disabled" ': '') +
                        'id="' + 'upload_tbl_' + task_id + '_' + task_type +
                        '" name="upload_tbl_list" class="form-control">';
                    option_list=option_list + '<option value="' + task["upload_tbl_list"]+ '" selected = "selected">' + task["upload_tbl_list"]+ '</option>';
                    // for(var oi=0; oi<sql_tbl_list.length; oi++){
                    //     if(sql_tbl_list[oi] == task["upload_tbl_list"]){
                    //         option_list=option_list + '<option value="' + sql_tbl_list[oi]+ '" selected = "selected">' + sql_tbl_list[oi]+ '</option>';
                    //     }else{
                    //         option_list=option_list + '<option value="' + sql_tbl_list[oi]+ '">' + sql_tbl_list[oi]+ '</option>';
                    //     }
                    // }
                    option_list=option_list + '</select>' + '</div>';
                    field_html.push(option_list);
...
//                    task_to_element_by_field(lastest_task_id, task, "command", "textarea", true),
    ...
    window.element_to_json = function($element) {
        ...
        var conn_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='conn_id']").val() || "";
        res["conn_id"] = conn_value;

        var src_conn_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='src_conn_id']").val() || "";
        res["src_conn_id"] = src_conn_value;
        var dst_conn_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='dst_conn_id']").val() || "";
        res["dst_conn_id"] = dst_conn_value;
        var tab_name_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='dst_tab_name']").val() || "";
        res["dst_tab_name"] = tab_name_value;


        var upload_csvfile_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='upload_csvfile']").val() || "";
        res["upload_csvfile"] = upload_csvfile_value;
        var upload_conn_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='upload_conn_id']").val() || "";
        res["upload_conn_id"] = upload_conn_value;
        var upload_tbl_value = $element.find(".tab-content[name='command'] .tab-pane.active :input[name='upload_tbl_list']").val() || "";
        res["upload_tbl_list"] = upload_tbl_value;
        return res;
    }
    ...
    window.form_to_conf = window.form_to_conf || function() {
        ...
        $("#tasks-container").children("div").each(function() {
            ...
            console.log('form_to_conf');
            console.log(element_to_json($(this)));
        });
        conf["tasks"] = tasks;
        return conf;
    };
    ...
    //     success_cb = success_cb || function() {};
    //     error_cb = error_cb || function() {alert('Failed!'); };
    //     console.log('refresh_url_template.replace(/DAG_ID/g, $("#dag_name").val())');
    //     console.log(refresh_url_template.replace(/DAG_ID/g, $("#dag_name").val()));
    //     $.ajax({
    ...
    var refresh_dag = function() {
        url = refresh_url_template.replace(/DAG_ID/g, $("#dag_name").val());
        postAsForm(url); return false
    };

    var view_dag = function() {
        // console.log('graph_url_template.replace(/DAG_ID/g, $("#dag_name").val())');
        // console.log(graph_url_template.replace(/DAG_ID/g, $("#dag_name").val()));
...
// });
// var factory_selectsrc = function(){
//     return function(){
//         console.log($('select[name="upload_conn_id"]'))
//         $('body').on("change",'select[name="upload_conn_id"]', function() {
//             alert( this.value );
//             console.log('this.value', this.value);
//         });
//
//     }
// }
// var fsr = factory_selectsrc()();


$(function () {
    $('body').on("change",'select[name="upload_conn_id"]', function() {
        // alert(this.value);
        cur_conn_id = $('select[name="upload_conn_id"]').find(":selected").text();
        task_name_id = $('#tasks-container :input[name="task_name"]').attr('value');

        upload_tbl_default = conf['tasks'].map(function (ts) {
            if (ts['task_name'] == task_name_id) {
                return ts['upload_tbl_list']
            }
        }).filter(function (e) {
            return e
        })

        // console.log('upload_tbl_default');
        // console.log(upload_tbl_default);
        $.ajax({
            url: 'api?api=get_tblnm_list&conn_id=' + this.value,
            type: 'GET',
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                // console.log(data.tblnm_list);
                window.sql_tbl_list = data.tblnm_list;
                $('select[name="upload_tbl_list"]').empty();
                for(var i=0; i<data.tblnm_list.length; i++){
                    if (data.tblnm_list[i] == upload_tbl_default) {
                        $('select[name="upload_tbl_list"]').append('<option value="' + data.tblnm_list[i] + '" selected="selected"> ' + data.tblnm_list[i] + '</option>');
                    }else {
                        $('select[name="upload_tbl_list"]').append("<option value='" + data.tblnm_list[i] + "'> " + data.tblnm_list[i] + "</option>");
                    }
                }
            },
            error: function () {
                window.sql_tbl_list = [];
                alert('Failed! 数据源不能正常连接');
            }
        });
    })
});
